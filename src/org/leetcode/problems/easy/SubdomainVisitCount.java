...
//            System.out.print(s + " ");
    ...
    public List<String> subdomainVisits(String[] cpdomains) {
        ...
        int caCount = 0;
        for(String key : cpdomains){
            String[] splits = key.split(" ");
            Integer num = Integer.parseInt(splits[0]);
            String domain = splits[1];

            String afterFirstDot = domain.substring(domain.lastIndexOf(".")+1);
            domainMap.put(afterFirstDot, domainMap.getOrDefault(afterFirstDot, 0)+num);
            if(domain.lastIndexOf(".") != domain.indexOf(".")){ // two dots exist
                String afterSecDot = domain.substring(domain.indexOf(".")+1);
                domainMap.put(afterSecDot, domainMap.getOrDefault(afterSecDot, 0)+num);
            }
        ...
        for(String key: domainMap.keySet()){
            resultsList.add(domainMap.get(key) + " " + key);
        }
